#Funsjon som finner det høyeste og laveste tallet i hver array
def MinMax(matrix):
    rows = len(matrix)
    columns = len(matrix[0])
    preMinPath = [matrix[0][0]]
    preMaxPath = [matrix[0][0]]
    for v in range(1, columns):
        preMinPath.append(matrix[0][v] + preMinPath[v-1])
        preMaxPath.append(matrix[0][v] + preMaxPath[v-1])
    for i in range(1, rows):
        minPath = [matrix[i][0] + preMinPath[0]]
        maxPath = [matrix[i][0] + preMaxPath[0]]
        for v in range(1, columns):
            min_sum = min(preMinPath[v], minPath[v-1])
            max_sum = max(preMaxPath[v], maxPath[v-1])
            minPath.append(matrix[i][v] + min_sum)
            maxPath.append(matrix[i][v] + max_sum)
        preMinPath = minPath
        preMaxPath = maxPath
    return preMinPath[columns-1], preMaxPath[columns-1]
