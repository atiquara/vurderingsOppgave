...
def MinMax(matrix):
    rows = len(matrix)  # Antall rader i matrisen
    columns = len(matrix[0])  # Antall kolonner i matrisen
    preMinPath = [matrix[0][0]]  # Liste som holder den foreløpige minste verdien for hver kolonne
    preMaxPath = [matrix[0][0]]  # Liste som holder den foreløpige største verdien for hver kolonne

    # Beregner den foreløpige minste og største verdien for hver kolonne i første rad
    for v in range(1, columns):
        preMinPath.append(matrix[0][v] + preMinPath[v-1])
        preMaxPath.append(matrix[0][v] + preMaxPath[v-1])

    # Itererer gjennom hver rad i matrisen fra den andre raden
    for i in range(1, rows):
        minPath = [matrix[i][0] + preMinPath[0]]  # Liste som holder den foreløpige minste verdien for hver kolonne i gjeldende rad
        maxPath = [matrix[i][0] + preMaxPath[0]]  # Liste som holder den foreløpige største verdien for hver kolonne i gjeldende rad

        # Beregner den foreløpige minste og største verdien for hver kolonne i gjeldende rad
        for v in range(1, columns):
            min_sum = min(preMinPath[v], minPath[v-1])  # Velger den minste verdien mellom den foreløpige minste verdien i forrige kolonne og den foreløpige minste verdien i gjeldende kolonne
            max_sum = max(preMaxPath[v], maxPath[v-1])  # Velger den største verdien mellom den foreløpige største verdien i forrige kolonne og den foreløpige største verdien i gjeldende kolonne
            minPath.append(matrix[i][v] + min_sum)  # Legger til den beregnede verdien for minste sti i gjeldende kolonne i minPath-listen
            maxPath.append(matrix[i][v] + max_sum)  # Legger til den beregnede verdien for største sti i gjeldende kolonne i maxPath-listen
    ...
    return preMinPath[-1], preMaxPath[-1]  # Returnerer den minste og største verdien for siste kolonne i matrisen
...
#Printer ut MinMax funsjonen med forskjellige arrays som variabler
print(MinMax(array1))
print(MinMax(array2))
print(MinMax(array3))
print(MinMax(array4))
