...
function MinMax(matrix)
  local rows = #matrix
  local colums = #matrix[1]
  local preMinPath = {matrix[1][1]}
  local preMaxPath = {matrix[1][1]}
  for v = 2, colums do
    preMinPath[v] = matrix[1][v] + preMinPath[v-1]
    preMaxPath[v] = matrix[1][v] + preMaxPath[v-1]
  end
  for i = 2, rows do
      local minPath = {matrix[i][1] + preMinPath[1]}
      local maxPath = {matrix[i][1] + preMaxPath[1]}
      for v = 2, colums do
          local min_sum = math.min(preMinPath[v], minPath[v-1])
          local max_sum = math.max(preMaxPath[v], maxPath[v-1])
          table.insert(minPath, matrix[i][v] + min_sum)
          table.insert(maxPath, matrix[i][v] + max_sum)
      end
      preMinPath = minPath
      preMaxPath = maxPath
  end
  return preMinPath[colums], preMaxPath[colums]
end
